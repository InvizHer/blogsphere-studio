      ...
      {categories.length > 0 && (
        <div className="flex flex-wrap gap-2">
          {categories.map((cat) => (
            <span
              key={cat}
              className="inline-flex items-center gap-1.5 rounded-full border border-primary/20 bg-primary/5 px-3.5 py-1.5 text-xs font-semibold uppercase tracking-wider text-primary transition-colors hover:bg-primary/10"
            >
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="relative">
          <button
            onClick={handleLike}
            disabled={liked}
            className={`group relative flex items-center gap-2 rounded-full border px-5 py-2.5 text-sm font-medium transition-all duration-300 ${
              liked
          ...
          >
            <motion.div animate={liked ? { scale: [1, 1.5, 1] } : {}} transition={{ duration: 0.4 }}>
              <Heart className={`h-4.5 w-4.5 transition-all ${liked ? "fill-red-500 text-red-500" : "group-hover:scale-110"}`} />
            </motion.div>
        ...
        <div className="relative">
          ...
          >
            <motion.div animate={disliked ? { scale: [1, 1.3, 1] } : {}} transition={{ duration: 0.3 }}>
              <ThumbsDown className={`h-4.5 w-4.5 transition-all ${disliked ? "fill-blue-500" : "group-hover:scale-110"}`} />
            </motion.div>
          </button>
